...
def test_coverage_by_changeset(coverage_responses, coverage_builds):
    ...
    with mock.patch('codecoverage_backend.api.q', Queue(connection=FakeStrictRedis(), is_async=False)):
        ...
        for changeset, expected in coverage_builds['info'].items():
            ...
            assert code == 200
            assert result['build_changeset'] == expected['build_changeset']
            ...
            assert result['git_build_changeset'] == expected['git_build_changeset']
            assert result['overall_cur'] == expected['overall_cur']
